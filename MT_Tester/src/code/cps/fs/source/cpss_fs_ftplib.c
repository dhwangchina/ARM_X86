...
#endif

int connect_timeout(int sock, struct sockaddr* addr, int size, int second);
...
#endif
    if (connect_timeout(sControl, (struct sockaddr *)&sin, sizeof(sin), 10) == -1)
    {
	perror("connect");
	net_close(sControl);
	return 0;
...
#endif
	if (connect_timeout(sData, &sin.sa, sizeof(sin.sa), 10) == -1)
	{
	    perror("connect");
	    net_close(sData);
	    return -1;
	...
	netbuf *nControl, int typ, int mode)
{
    ...
    {
    	if(0 == strncmp(nControl->response, "550", 3))
    	{
    		g_lCpsFsFtError = CPS_FS_FT_ERROR_REMOTE_FILE;
    	}
    	if(0 == strncmp(nControl->response, "553", 3))
    	{
			g_lCpsFsFtError = CPS_FS_FT_ERROR_PERMISSION_DENIED;
    	}
			return 0;
    }
...
int connect_timeout(int sock, struct sockaddr* addr, int size, int second)
{
	...
	if(-1 == Ret)
	{
		...
		FD_ZERO(&Fd);
		FD_SET(sock, &Fd);
		stTime.tv_sec = second;
		select(sock + 1, NULL, &Fd, NULL, &stTime);
		if(0 == FD_ISSET(sock, &Fd))
		{
			return -1;
		}
	}
